...
const removeElement = (element, delay) => {
    setTimeout(() => {
        ...
        if (container && container.contains(element)) {
            ...
            console.error("Element not found in container / container missing");
        }
    }, delay);
};
...
const createStar = position => {
    ...
    star.className = "star fa fa-book";

    star.style.left = px(position.x + scrollOffsets.x);
    star.style.top = px(position.y + scrollOffsets.y);
...
const createGlowPoint = position => {
    ...
    glow.style.left = px(position.x + scrollOffsets.x);
    glow.style.top = px(position.y + scrollOffsets.y);
...
const updateMouseTrailContainerHeight = () => {
    ...
    if (mouseTrailContainer) {
        mouseTrailContainer.style.height = height + 'px';
    }
};

updateMouseTrailContainerHeight();
window.addEventListener('resize', updateMouseTrailContainerHeight);
window.addEventListener('scroll', updateMouseTrailContainerHeight);
